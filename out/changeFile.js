...
exports.changeFile = (path, priorityExtension, callback) => {
    let custom = priorityExtension !== ".ts"
    ...
    function (error, data) {
        ...
        if (data) {
            const newData = data.replace(/(?<=function tryAddingExtensions)(.*)(?=function tryExtension)/gms, replacement);
            ...
            vscode.window.showInformationMessage("Can`t find tsserver.js in workspaces node_modules.");
        }
    });
};
